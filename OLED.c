...
以下宏定义在main.h中定义，需要在Cubemx配置软件I2C引脚时将引脚的"user lable"分别设置为SCL_Pin和SDA_Pin。

GPIO_InitStruct.Pin = SCL_Pin|SDA_Pin;
GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;
PIO_InitStruct.Pull = GPIO_PULLUP;
...
使用软件I2C需要将硬件I2C宏定义注释掉，将软件I2C那行的注释取消。
...
/*OLED从机地址*/
#define OLED_ADDRESS 0x78	// 0x3C是OLED的7位地址，左移1位最后位做读写位变成0x78
...
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStruct.Pin = OLED_SCL;                 // 设置引脚
 	HAL_GPIO_Init(OLED_SCL, &GPIO_InitStruct);// 初始化GPIO

    GPIO_InitStruct.Pin = OLED_SDA;
    HAL_GPIO_Init(OLED_SDA, &GPIO_InitStruct);
